...
# ---------------------------------------------------------------------------
# Dummy classes to simulate a valid MQTT PUBLISH packet structure from Scapy.
# ---------------------------------------------------------------------------
...
def test_run_with_input_output_handlers():
    """
...
def test_run_with_mqtt_topic_prefix(monkeypatch):
    """
    Test that run() correctly applies MQTT topic prefix.
    """
    # Set global MQTT topic prefix
    monkeypatch.setattr(sm, '_mqtt_topic_prefix', 'solarflow')
...
def test_run_with_blacklisted_topic(monkeypatch):
    """
    Test that run() correctly filters out blacklisted topics.
    """
    # Set global topics blacklist
    monkeypatch.setattr(sm, '_topics_blacklist', {'/test/topic'})
...
def test_output_handler_connection_failure():
    """
...
def test_input_handler_factory(tmp_path):
    ...
    live_handler = sm.InputHandlerFactory.create_handler(sm.InputType.INTERFACE, source="wlan0")
    assert isinstance(live_handler, sm.LiveInputHandler)
    ...
    pcap_handler = sm.InputHandlerFactory.create_handler(sm.InputType.PCAP_FILE, source=str(tmp_file))
    assert isinstance(pcap_handler, sm.PcapInputHandler)
...
def test_output_handler_factory():
    ...
    mqtt_handler = sm.OutputHandlerFactory.create_handler(
        sm.OutputType.MQTT,
        mqtt_host="localhost",
        mqtt_port=1883,
        mqtt_user="test",
    ...
    assert isinstance(mqtt_handler, sm.MqttOutputHandler)
    assert mqtt_handler.host == "localhost"
    assert mqtt_handler.port == 1883

    # Test STDOUT output handler creation
    stdout_handler = sm.OutputHandlerFactory.create_handler(sm.OutputType.STDOUT)
    assert isinstance(stdout_handler, sm.StdoutOutputHandler)
